...
class PID: 
    ...
    def __init__(self,K_p=1.0,K_i=0.0,K_d=0):
        self.kp=K_p
        self.ki=K_i
        self.kd=K_d
...
def run_demo():
    ...
    init_flag_dist = robot.getDistanceToFlag()
    pid= PID(0.1,0.5,0.5)

    while not rospy.is_shutdown():
        # Strategy 2
        Obs_dist=robot.get_sonar()

        flag_dist=robot.getDistanceToFlag()

        #speed and angle update
        speed=pid.PID_Calculator(flag_dist)
        angle= pid.PID_Calculator(robot.yaw)

        #obstacle detection

        if Obs_dist<0.5:
            speed=0
            angle=pid.PID_Calculator(0.5)
        else :
